...
final class UnderstoryRecord {
  const UnderstoryRecord({
...
final class UnderstoryRepository {
  const UnderstoryRepository({required Database database})
    : _database = database;

  final Database _database;

  Future<int> insert(UnderstoryRecord record) {
    return _database.insert('understory', _toRow(record));
  }

  Future<void> update(UnderstoryRecord record) async {
    final id = record.id;
    if (id == null) {
      return;
    }
  ...
  Future<List<UnderstoryRecord>> getByProbaInfoId(int probaInfoId) async {
  ...
  Future<void> deleteById(int id) async {
    await _database.delete('understory', where: 'id = ?', whereArgs: [id]);
  }

  UnderstoryRecord _fromRow(Map<String, Object?> row) {
  ...
  Map<String, Object?> _toRow(UnderstoryRecord record) {
    return {
