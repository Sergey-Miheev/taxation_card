...
final class _UndergrowthScreenState extends State<UndergrowthScreen>
  ...
  Future<void> _openAddDialog(int probaInfoId) async {
    ...
    _showTopSnackBar('Запись подроста добавлена.');
  }
  ...
  Future<void> _showPlotSetupDialog(int selectedProbaInfoId) async {
    ...
    final setup = await showDialog<_PlotSetupDraft>(
      ...
      builder: (context) => _PlotSetupDialog(
        title: 'Учетные площадки подроста',
...
final class _TotalPlotAreaLabel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final text = area == null
        ? 'Общая площадь подроста: -, га'
        : 'Общая площадь подроста: ${_formatDouble(area)}, га';
              ...
              Row(
                children: [
                  _HeaderBox(
                    ...
                    text: 'Мелкий, средний подрост',
                    theme: theme,
                  ),
                  _HeaderBox(
                    width: largeGroupWidth,
                    height: topHeaderHeight,
                    text: 'Крупный подрост',
                    theme: theme,
                  ),
                ],
              ),
